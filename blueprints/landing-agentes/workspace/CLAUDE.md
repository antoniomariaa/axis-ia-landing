...
Sitio de marketing de Axis IA: tres landings estáticas, sin build, sin framework, sin backend.

## Commands

| Task | Command |
|---|---|
| Servir localmente | `npx --yes http-server . -p 8080` — abre `http://localhost:8080/` (landing 1), `.../inmobiliarias/` (landing 2) o `.../agentes/` (landing 3). Requiere Node/npm; no requiere Python |
| Verificar ruta | `curl -s -o /dev/null -w '%{http_code}' http://localhost:8080/<landing>/` — debe imprimir `200` (`<landing>` es `inmobiliarias` o `agentes`) |
| Buscar contenido | `grep -q '<patrón>' <landing>/index.html` |
| Contar CTAs de WhatsApp | `grep -c "584121484033" <landing>/index.html` — debe dar `3` en cada landing con esa convención |
| Confirmar una landing intacta | `git diff --stat -- <landing>/index.html` (o `index.html` para la raíz) — debe no mostrar salida |
...
**Tres landings, un CSS compartido:**

- `index.html` (raíz) — landing 1, auditoría gratuita general. **Nunca la edites** salvo que te lo
  pidan explícitamente por su nombre.
- `inmobiliarias/index.html` — landing 2, vende el agente de WhatsApp a **dueños de agencias
  inmobiliarias** (lenguaje de equipo/reparto de leads/panel del jefe). Assets reales pendientes
  (video, capturas) en `inmobiliarias/assets/` — ver el README ahí dentro.
- `agentes/index.html` — landing 3, vende el mismo agente de WhatsApp pero a un **agente
  inmobiliario individual** (no a un dueño de agencia) — tono en primera persona, tuteo neutro
  venezolano, sin lenguaje de equipo ni panel gerencial. Mensaje de WhatsApp propio ("...para mis
  leads"), distinto al de `inmobiliarias/index.html` ("...para mi inmobiliaria"). Sin carpeta
  `assets/` propia — su mockup de producto es permanentemente el patrón CSS de burbujas de chat, sin
  video real planeado.
- `assets/shared.css` — tokens, reset, nav, botones, shell de sección, `.hero`/`.stage`/`.tilt`,
  `.glance-*`, `.report-card`/`.rc-*` y footer. **Todo lo que ya era idéntico byte a byte entre
  landings vive acá.** Enlazado como `assets/shared.css` desde `index.html` y `../assets/shared.css`
  desde `inmobiliarias/index.html` y `agentes/index.html`.

**No hay cross-linking entre ninguna de las tres landings** — ninguna enlaza a otra. Cada canal de
tráfico lleva a una sola oferta.

**CSS/JS propio de cada landing sigue inline** en su `<style>` — solo lo que ya era 100% idéntico
entre landings se extrajo a `assets/shared.css` (ver arriba). Nunca crees un tercer archivo CSS
compartido ni dupliques un bloque que ya vive en `shared.css` de vuelta a un `<style>` inline. Sin
JS en ningún lado — ni compartido ni por landing.
...
| Hero compartido (`.hero`/`.hero-text`/`.stage`/`.tilt`) | Bloque de texto a la izquierda + mockup con perspectiva y resplandor abajo — mismo patrón en las tres landings, contenido propio de cada una |
...
5. **Todo enlace de contacto es un `https://wa.me/<numero>?text=<mensaje-codificado>`**, nunca un
   ...
   inventes ni los generalices a un placeholder, y nunca copies el mensaje de una landing a otra.
6. **Nada de métricas o resultados de clientes inventados.** Los mockups de producto usan etiquetas
   cualitativas ("Presupuesto capturado", "Recomendada"), nunca cifras — no hay datos reales que las
   respalden.
...
8. **Tuteo neutro venezolano en todo el copy nuevo, nunca voseo** ("tenés", "querés", "podés",
   "sabés", "escribís", "sos"). Verifica con un grep negativo antes de dar una tarea por terminada.

## Design system

Definido una sola vez en el `:root` de `assets/shared.css`; cada landing lo hereda vía `<link>`.
...
| `.claude/rules/agentes-landing.md` | `agentes/**` |
...
5. **Nunca se agrega un link entre ninguna de las landings del sitio** (`index.html`,
   `inmobiliarias/index.html`, `agentes/index.html`) — cada una vive sola.
